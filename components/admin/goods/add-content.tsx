...
import { AddContentSchema } from "@/lib/db/content/schema";
import { addContent } from "@/lib/db/content/actions";
...
export function AddContent({
  ...
  async function onSubmit(values: z.infer<typeof AddContentSchema>) {
    ...
    const action = await addContent(values);

    if (action?.message) {
      if (action?.errors) {
        Object.entries(action.errors).forEach(([key, value]) => {
          form.setError(key as keyof z.infer<typeof AddContentSchema>, {
            message: value[0],
          });
        });
      }
